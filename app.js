...
async function api(action, payload = {}) {
  ...
  const res = await fetch(API_URL, { method: "POST", body: JSON.stringify({ action, code: state.code, ...payload }) });
  const data = await res.json();
  if (!data.ok) throw new Error(data.error || "ไม่สามารถเชื่อมต่อฐานข้อมูลได้");
  return data;
}
